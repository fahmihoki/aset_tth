...
<body>
	...
	<form method="GET" action="search_alat_ukur.php" style="text-align: center;">
		<label style="color: white;">Search Data : </label>
    ...
    <div style="overflow-x: auto;" class="card-body bg-primary">
                <table class="table table-bordered bg-white">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center text-dark">Asset Name</th>
				...
				$no = 1;
							...
							$datas = mysqli_query($koneksi, "SELECT * FROM alat_ukur ORDER BY id_alat_ukur DESC LIMIT 10") or die(mysqli_error($koneksi));
					...
					$query = "SELECT * FROM alat_ukur WHERE no_aset like '%".$kata_cari."%' OR nama_alat_ukur like '%".$kata_cari."%' OR merek like '%".$kata_cari."%' OR tipe_model like '%".$kata_cari."%' OR no_seri like '%".$kata_cari."%' ORDER BY id_alat_ukur DESC";
				} else {
					//jika tidak ada pencarian, default yang dijalankan query ini
					$query = "SELECT * FROM alat_ukur ORDER BY id_alat_ukur DESC";
				}
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					<tr>
                        ...
                        <td class="text-center text-dark"><?= $row['no_aset']; ?></td>
                        <td class="text-center text-dark"><?= $row['nama_alat_ukur']; ?></td>
                        <td class="text-center text-dark"><?= $row['merek']; ?></td>
                        <td class="text-center text-dark"><?= $row['tipe_model']; ?></td>
                        <td class="text-center text-dark"><?= $row['no_seri']; ?></td>
                        <!-- <td><?= $row['buku_manual']; ?></td> -->
                        <td class="text-center text-dark">
                                <a href="edit_alat_ukur.php?id_alat_ukur=<?= $row['id_alat_ukur']; ?>" class="btn btn-sm btn-warning">Edit</a>
                                <a href="hapus_alat_ukur.php?id_alat_ukur=<?= $row['id_alat_ukur']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('anda yakin ingin hapus?');">Hapus</a>
                        </td>
                    </tr>
